...
class Classifier:
    ...
    def __init__(self, trt_engine='models/classification/engine.trt', labels='models/classification/labels.txt', labels_used='models/classification/labels_used.txt', max_batch_size=5):
        ...
        self.images_pool = [jetson_utils.cudaAllocMapped(
        ...
        with open(labels_used, 'r') as f:
            ...
            self.labels_used_map = {line.strip().split(
                ':')[0]: line.strip().split(':')[1] for line in f}
            # Ignore classes not listed in labels_used.
            self.labels_not_used_indexes = np.where(
                ~np.isin(self.labels, list(self.labels_used_map.keys())))[0]
    ...
    def preprocess_images(self, imgs):
        ...
        batch_images_np = np.empty(
            (len(batch_images), 224, 224, 3), dtype=np.float32)
        # Convert all images to NumPy arrays and copy directly into the pre-allocated array
        for i, img in enumerate(batch_images):
            batch_images_np[i] = jetson_utils.cudaToNumpy(
    ...
    def _classify(self, imgs):
        ...
        self.context.set_binding_shape(0, (batch_size,) + INPUT_SHAPE)
        ...
        self.context.execute(
            bindings=[int(preprocessed_images.ptr), int(d_output)])
        ...
        predictions = [self.labels_used_map[self.labels[index]]
                       for index in max_confidence_indexes]
        postprocessing_time = (time.time() - st) * 1000

        self.logger.debug('Classifier Total Time: {} [msec] (batch size: {})'.format(
            preprocessing_time + inference_time + postprocessing_time, batch_size))
        self.logger.debug(
            'Preprocessing: {} [msec]'.format(preprocessing_time))
        self.logger.debug('Inference: {} [msec]'.format(inference_time))
        self.logger.debug(
            'Postprocessing: {} [msec]'.format(postprocessing_time))

        return predictions

    def classify(self, imgs):
        # Batch imgs based on max_batch_size
        batches = [imgs[i:i + self.max_batch_size]
