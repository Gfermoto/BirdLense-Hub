...
- **Backend кэширование ([#203](https://github.com/Gfermoto/BirdLense-Hub/issues/203)):** `services/cache.py` — in-memory TTL или **Redis** при `REDIS_URL` (несколько воркеров/реплик). `/api/ui/overview` и region-comparison — как ранее; **тяжёлые эндпоинты «Система» и хранилище** (метрики, посетители, история, активность, `storage_stats`) — TTL-кэш с инвалидацией после purge/retention/scan/clean visits и merge видов. **Nginx Brotli** — динамический модуль `ngx_brotli`, собирается в образе (Ubuntu 22.04 без пакета `libnginx-mod-http-brotli`). **PostgreSQL:** `DATABASE_URL` + `SQLALCHEMY_POOL_SIZE` / `SQLALCHEMY_MAX_OVERFLOW` в `config.py`. Пример стека: `app/docker-compose.stack.example.yml`. Зависимости: `redis`, `psycopg[binary]`.
