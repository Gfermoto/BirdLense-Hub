...
- **Документация:** актуализация ROADMAP EN/RU (апрель 2026 в заголовках backlog, UI видов **v0.3.7+**, уточнение формулировки outcome эпика реестра), SECURITY EN/RU (дата обновления и baseline gitleaks — апрель 2026), индекс [docs/README](docs/README.md) / [README.ru](docs/README.ru.md) со строкой эпика CV/ML [#367](https://github.com/Gfermoto/BirdLense-Hub/issues/367), [CV_ML_PREP](docs/CV_ML_PREP.md) / [RU](docs/CV_ML_PREP.ru.md) — явная ссылка на эпик **#367**, [HUB_EPICS_TRACKER](docs/HUB_EPICS_TRACKER.md) / [RU](docs/HUB_EPICS_TRACKER.ru.md) — секция параллельного трека CV/ML; журнал [VERIFICATION](docs/VERIFICATION.md) / [RU](docs/VERIFICATION.ru.md) — запись о проверках `check-docs-version.py`, `check_site_map_meta_paths.py`, `mkdocs build --strict`.
...
- **Синхронизация с кодом и CI:** обновлены [TESTING.md](docs/TESTING.md) / [TESTING.ru.md](docs/TESTING.ru.md) (полный перечень job workflow CI), [ARCHITECTURE](docs/ARCHITECTURE.md) / [RU](docs/ARCHITECTURE.ru.md) (Alembic/Flask-Migrate), [REPOSITORY_LAYOUT](docs/REPOSITORY_LAYOUT.md) / [RU](docs/REPOSITORY_LAYOUT.ru.md) (`create_app`, migrations, services), [ROADMAP](docs/ROADMAP.md) / [RU](docs/ROADMAP.ru.md) (версии React/Vite из `package.json`, строка про схему БД, волна maintainability [#292](https://github.com/Gfermoto/BirdLense-Hub/issues/292)–[#297](https://github.com/Gfermoto/BirdLense-Hub/issues/297) + security [#277](https://github.com/Gfermoto/BirdLense-Hub/issues/277)–[#287](https://github.com/Gfermoto/BirdLense-Hub/issues/287)), [README](docs/README.md) / [RU](docs/README.ru.md), [Documentation](docs/Documentation.md) / [RU](docs/Documentation.ru.md) (чеклист ревью), [VERSIONING](docs/VERSIONING.md) / [RU](docs/VERSIONING.ru.md), корневые [CONTRIBUTING](CONTRIBUTING.md) / [RU](CONTRIBUTING.ru.md). Ссылки на корневой `CHANGELOG` и `.github/workflows/ci-pr.yml` ведут на GitHub, чтобы **`mkdocs build --strict`** не ругался на пути вне `docs/`.
...
- **Web (tech debt #198 закрыт):** публичные **`/api/ui/*`** вынесены из монолита в доменные модули **`ui_*_routes`**; **`ui_routes.py`** — только **`register_routes`** и реэкспорт хелперов таймлайна (`build_merged_timeline_items`, `parse_timeline_iso`). См. [ARCHITECTURE](docs/ARCHITECTURE.ru.md) ([#198](https://github.com/Gfermoto/BirdLense-Hub/issues/198)).
