...
- **Видео / таймлайн:** листание prev/next с записей открытых **с таймлайна** (`state.from` содержит `/timeline`) использует `neighbor_mode=visit_day` — только ролики, привязанные к визитам за локальный день, а не все файлы за сутки; контекст `from`/`visitId` сохраняется при шагах. С других страниц без таймлайна — по-прежнему полный список за день; с `visitId` без таймлайна — только ролики **этого** визита (`neighbor_mode=visit`). API: режимы `visit_day`, `visit`.
