...
export const DailyPatternChart: React.FC<DailyPatternChartProps> = ({
  ...
  // Helper function to get detection value adjusted for timezone
  const getOffsetValue = (detections: number[], hour: number) => {
    // Add offset since detections array is in UTC and we're converting from local
    const utcHour = (hour + offsetInHours + 24) % 24;
    return detections[Math.floor(utcHour)];
  };

  // Calculate max detections using timezone-adjusted values
  ...
  const handleArcClick = (speciesId: number, hour: number) => {
    // Set the hour on the existing date
  ...
  return (
    <Box
      sx={{ width: size, height: size, position: 'relative', margin: 'auto' }}
    >
