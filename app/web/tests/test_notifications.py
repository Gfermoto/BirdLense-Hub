...
def test_notify_returns_text_fallback_reason_when_photo_send_fails(monkeypatch):
    ...
    monkeypatch.setattr(notifications_mod, "_telegram_proxy_mode", lambda: "none")
    monkeypatch.setattr(
        notifications_mod, "_compress_image_for_telegram", lambda image_bytes, aggressive=False: image_bytes
    )
    monkeypatch.setattr(
        notifications_mod,
        "_telegram_request",
...
def test_notify_detections_logs_decode_failed_reason(app, client, monkeypatch):
    ...
    with app.app_context():
        ...
        response = client.post(
            "/api/processor/notify/detections",
            json={
        ...
        row = (
            ...
            .filter(ActivityLog.type == "notify_preview")
