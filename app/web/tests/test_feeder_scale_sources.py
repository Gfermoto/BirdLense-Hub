...
def test_scale_tare_available_for_esphome_direct():
    g = _cfg_get(
        {
            "integrations.scales.enabled": True,
            "integrations.scales.source": "esphome_direct",
            "integrations.scales.esphome_url": "http://scale.local",
            "integrations.scales.esphome_tare_button_id": "tare_scale",
        }
    )
...
def test_trigger_scale_tare_esphome_direct_posts_button(monkeypatch):
    seen: list[str] = []
    ...
    assert ok is True
    assert msg == "ok"
    assert seen == ["http://scale.local/button/tare_scale/press"]


def test_get_feeder_scale_snapshot_esphome_direct_reads_weight_and_bird_present(monkeypatch):
    def fake_get(url, timeout):
        if url.endswith("/sensor/feeder_weight"):
            return _Resp({"state": "12.34", "unit_of_measurement": "kg"})
    ...
    g = _cfg_get(
        {
            ...
            "integrations.scales.source": "esphome_direct",
            "integrations.scales.esphome_url": "http://scale.local",
            "integrations.scales.esphome_weight_sensor_id": "feeder_weight",
            "integrations.scales.esphome_bird_present_sensor_id": "bird_present",
            "integrations.scales.unit": "kg",
        }
    )
    ...
    assert snap["source"] == "esphome_direct"
