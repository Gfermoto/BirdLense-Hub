...
def _get_button_custom_emoji_id(tags):
    ...
    key = (
        ...
        if tags == "chipmunk"
        else ("custom_emoji_id_bird" if tags == "bird" else "custom_emoji_id_open_live")
    )
    ...
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                delay = 2**attempt
                logging.warning(
                    "Telegram attempt %d/%d failed (%s), retry in %ds", attempt + 1, retries, type(e).__name__, delay
                )
                time.sleep(delay)
    raise last_exc
...
def _telegram_send_message(
    token, chat_id, text, link=None, button_emoji="📺", button_style="primary", button_tags=None, **kwargs
):
    ...
    payload = {
        ...
        "parse_mode": "HTML",
        "disable_notification": app_config.get("notifications.disable_notification", False),
        "protect_content": app_config.get("notifications.protect_content", False),
    ...
    if link:
        ...
        payload["reply_markup"] = {
            "inline_keyboard": [
                [_telegram_button_open_live(link, button_emoji, button_style, icon_custom_emoji_id=custom_id)]
            ]
        }
            ...
            if _telegram_proxy_mode() == "mtproto":
                ...
                def _bot_api_send_photo(photo_bytes):
                            ...
                            "inline_keyboard": [
                                [
                                    _telegram_button_open_live(
                                        link_url, button_emoji, "primary", icon_custom_emoji_id=custom_id
                                    )
                                ]
                            ]
                        }
                    ...
                    if view_stars > 0:
                        ...
                        data["media"] = json.dumps([{"type": "photo", "media": "attach://photo"}])
                        return _telegram_request(
                            "POST",
                            f"{base}/bot{token}/sendPaidMedia",
