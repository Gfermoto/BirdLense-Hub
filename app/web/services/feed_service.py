...
def check_esphome_reachable():
    ...
    if source != "esphome":
        return "not_configured"
    url = os.environ.get("ESPHOME_FEEDER_URL") or app_config.get("feed.esphome_url", "http://feeder.local")
    if not url:
        return "not_configured"
    try:
...
def dispense_feed():
    ...
    if source == "mqtt":
        ...
        url = os.environ.get("ESPHOME_FEEDER_URL") or app_config.get("feed.esphome_url", "http://feeder.local")
        entity_id = os.environ.get("ESPHOME_SWITCH_ID") or app_config.get("feed.esphome_switch_id", "bird_feeder")
        ...
        try:
            if esphome_type == "button":
                r = requests.post(f"{url.rstrip('/')}/button/{entity_path}/press", timeout=5)
                r.raise_for_status()
                logger.info("Feed dispensed via ESPHome (button press)")
            else:
                r = requests.post(f"{url.rstrip('/')}/switch/{entity_path}/turn_on", timeout=5)
                r.raise_for_status()
                time.sleep(duration)
                requests.post(f"{url.rstrip('/')}/switch/{entity_path}/turn_off", timeout=5)
