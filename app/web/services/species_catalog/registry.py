...
Реализация в ``services/species_catalog/registry.py``; shim —
``services/species_registry_service.py`` (#344 фаза C).
"""
...
def unresolved_species_report(limit: int = 100) -> list[dict]:
    ...
    rows = (
        SpeciesUnresolvedName.query.order_by(
            SpeciesUnresolvedName.seen_count.desc(), SpeciesUnresolvedName.last_seen_at.desc()
        )
...
def enrich_species_metadata_with_status(
    ...
    for sp in q.all():
        ...
        if _has_metadata_text(sp.image_url) and _has_metadata_text(sp.description) and not retry_failed_only:
            skipped += 1
            if not dry_run:
                db.session.commit()
            continue
        ...
        try:
            ...
            if cached and (not _has_metadata_text(sp.image_url) or not _has_metadata_text(sp.description)):
                img_c, desc_c = cached
...
def repair_catalog_cards(
    ...
    *,
    ...
    still_missing = sum(1 for sp in targets if not (sp.image_url or "").strip() or not (sp.description or "").strip())
    if dry_run:
        db.session.rollback()
    else:
        db.session.commit()
