...
Реализация: ``services/dataset_export/export_core.py``; shim —
``services/dataset_export_service.py`` (#344).
"""
...
def _is_likely_fullframe(width: int, height: int) -> bool:
    ...
    if pixels < MIN_PIXELS_FULLFRAME:
        return False
    aspect = width / height
    return abs(aspect - ASPECT_16_9) < ASPECT_TOLERANCE or abs(aspect - ASPECT_4_3) < ASPECT_TOLERANCE
...
def _split_grouped_train_val_test(
    ...
    *,
    ...
    for idx, (group_key, rows) in enumerate(grouped_rows):
        ...
        if not force_train and target_test > 0 and (test_count < target_test or not test_rows):
            test_rows.extend(rows)
            split_groups["test"].append(group_key)
            test_count += len(rows)
            continue
        if not force_train and target_val > 0 and (val_count < target_val or not val_rows):
    ...
    return (
        ...
        test_rows,
        {
            ...
            "groups_per_split": {key: len(value) for key, value in split_groups.items()},
        },
    )
...
def _quality_report_from_entries(
    ...
    duplicate_tracks = sorted(f"{a}_{b}" for (a, b), locs in track_locations.items() if len(locs) > 1)
...
def _manually_corrected_video_tracks() -> set[tuple[int, int]] | None:
    ...
    rows = (
        ...
        .with_entities(VideoSpecies.video_id, VideoSpecies.track_id)
        .all()
    )
    if not rows:
        return set()
...
def build_dataset_zip(
    ...
    if not os.path.isdir(dataset_base):
        return None, ('Dataset folder not found. Enable "Save dataset crops" and record videos.')
    ...
    with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
        if ready_for_train:
            train_source = _iter_filtered_class_images(os.path.join(dataset_base, "train"))
                ...
                try:
                    ...
                    for gk, mapping in groups_map.items():
                        ...
                        preferred = max(
                            sorted(
                                counts.items(),
                                key=lambda x: ("train", "val", "test").index(x[0])
                                if x[0] in ("train", "val", "test")
                                else 3,
                            ),
                            key=lambda x: x[1],
                        )[0]
        ...
        try:
            ...
            for ent in export_entries:
                split, _cls, fname, gk = (
                    ...
                    ent[1],
                    ent[2],
                    (ent[3] if len(ent) > 3 else _group_key_for_filename(ent[2], video_meta)),
                )
            ...
            for gk, rows in group_to_entries.items():
                ...
                preferred = max(
                    sorted(
                        splits.items(),
                        key=lambda x: ("train", "val", "test").index(x[0]) if x[0] in ("train", "val", "test") else 3,
                    ),
        ...
        if strict_quality:
            gl = (
                (quality.get("group_leakage") or {}).get("train_val_shared", 0)
                    ...
                    with zipfile.ZipFile(buf2, "w", zipfile.ZIP_DEFLATED) as zf2:
                        for split, lst in (("train", new_tr), ("val", new_va), ("test", new_te)):
                            for _, cls, fname in lst:
                                src = os.path.join(
                                    dataset_base, split if split != "train" else "train", cls, os.path.basename(fname)
                                )
                                try:
        ...
        info["manifest"] = {
            ...
            "split_params": {
                ...
                "grouped_split_strategy": ("recording_day_or_video" if ready_for_train else "pre_split_direct_export"),
            },
        }
        fp_src = json.dumps(
            {
        ...
        zf.writestr("dataset_info.json", json.dumps(info, ensure_ascii=False, indent=2))
...
def move_crop_on_species_correction(
    ...
    if len(matches) != 1:
        if matches:
            logger.debug("Skip move: %d matches for video_id=%s track_id=%s", len(matches), video_id, track_id)
        return False
    src = matches[0]
...
__all__ = [
    ...
    "ASPECT_4_3",
    "ASPECT_TOLERANCE",
    "MIN_PIXELS_FULLFRAME",
    "data_dir",
    "_delete_dataset_crops_for_video_ids",
    "_get_image_dimensions",
    "_group_key_for_filename",
    "_is_likely_fullframe",
    "_manually_corrected_video_tracks",
    "_parse_video_track_from_filename",
    "_quality_report_from_entries",
    "_sanitize_dirname",
    "_season_key",
    "_slice_report_from_entries",
    "_split_grouped_train_val_test",
    "_split_train_val_test",
    "_video_ids_in_period",
    ...
    "retro_export_all_video_detections",
]
