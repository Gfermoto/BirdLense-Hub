...
def _copy_missing_metadata(source: Species, target: Species) -> None:
    if not (target.description or "").strip() and (source.description or "").strip():
        target.description = source.description
    if not (target.image_url or "").strip() and (source.image_url or "").strip():
        target.image_url = source.image_url
    if not (target.metadata_source or "").strip() and (source.metadata_source or "").strip():
        target.metadata_source = source.metadata_source
    if not (target.metadata_source_url or "").strip() and (source.metadata_source_url or "").strip():
