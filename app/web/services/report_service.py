...
def get_monthly_report_data(session, start_dt, end_dt):
    ...
    top_query = (
        session.query(
            ...
            *[
                func.sum(case((func.strftime("%H", SpeciesVisit.start_time) == str(h).zfill(2), 1), else_=0)).label(
                    f"detection_hour_{h}"
                )
                for h in range(24)
            ],
    ...
    stats_row = (
        session.query(
            ...
            func.avg(func.strftime("%s", SpeciesVisit.end_time) - func.strftime("%s", SpeciesVisit.start_time)).label(
                "avgVisitDuration"
            ),
        )
    ...
    video_dur_expr = func.strftime("%s", Video.end_time) - func.strftime("%s", Video.start_time)
    recording_sec = (
    ...
    dur_expr = case(
        (VideoSpecies.end_time >= VideoSpecies.start_time, VideoSpecies.end_time - VideoSpecies.start_time), else_=0
    )
    audio_duration = (
