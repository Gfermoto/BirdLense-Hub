...
def build_species_summary(session, species, children, all_species_ids: list) -> dict:
    ...
    weather_stats = (
        ...
        .group_by(func.round(Video.weather_temp), Video.weather_clouds)
        .all()
    )
    ...
    food_stats = (
        ...
        .join(video_bird_food_association, BirdFood.id == video_bird_food_association.c.birdfood_id)
        .join(Video, Video.id == video_bird_food_association.c.video_id)
        .join(VideoSpecies, VideoSpecies.video_id == Video.id)
        .join(SpeciesVisit, VideoSpecies.species_visit_id == SpeciesVisit.id)
        .filter(SpeciesVisit.species_id.in_(all_species_ids))
        .group_by(BirdFood.name)
        .order_by(func.count(func.distinct(SpeciesVisit.id)).desc())
        .limit(5)
        .all()
    )
