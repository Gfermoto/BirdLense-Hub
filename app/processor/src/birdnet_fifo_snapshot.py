"""Периодический снимок FIFO BirdNET на диск для UI (процессор и веб — разные процессы)."""
...
RELATIVE_PATH = os.path.join("diagnostics", "birdnet_fifo_snapshot.json")
...
def sanitize_birdnet_event_for_export(ev: dict) -> dict:
    """Только публичные поля события, без внутренних ключей (_ts_epoch и т.д.)."""
    out: dict = {}
...
def write_birdnet_fifo_snapshot(
    *,
    data_dir: str,
    events: list[dict],
    fifo_cap: int,
    ...
    """Атомарно записать снимок под data_dir/diagnostics/."""
    try:
        recent_limit = int(app_config.get("processor.birdnet_fifo_snapshot_recent_limit") or 80)
    except (TypeError, ValueError):
        recent_limit = 80
    payload = build_birdnet_fifo_snapshot_payload(
        events=events,
        fifo_cap=fifo_cap,
    ...
    try:
        ...
        logger.warning("BirdNET FIFO snapshot: cannot mkdir %s: %s", diag_dir, e)
        return
    target = os.path.join(data_dir, RELATIVE_PATH)
    try:
        raw = json.dumps(payload, ensure_ascii=False, separators=(",", ":"))
        fd, tmp = tempfile.mkstemp(
            prefix="birdnet_fifo_",
            suffix=".json",
            dir=diag_dir,
        )
        try:
