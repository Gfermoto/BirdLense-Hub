...
def resolve_motion_recording_camera_id(
    ...
    *,
    ...
    triggered_by = str(
        getattr(motion_detector, 'get_triggered_by', lambda: None)() or ''
    ).strip().lower()
    if triggered_by == 'opencv' and mqtt_aggregator is not None:
        try:
            ...
            proc_cams = cameras_for_processor(
                get_valid_cameras(app_config.get('video.cameras') or [])
            )
            camera_ids = [
        ...
        if camera_ids:
            try:
                max_age = float(
                    app_config.get('processor.frigate_activity_hold_seconds') or 6.0
                )
            ...
            if callable(pick_fn):
                ...
                if picked:
                    logger.info(
                        'OpenCV trigger: resolved recording camera=%s '
                        'from recent Frigate MQTT (candidates=%s)',
                        picked,
                        camera_ids,
                    )
                    return str(picked)
