...
class FrameProcessor:
    def __init__(self, detection_strategy: DetectionStrategy, save_images=False, tracker='bytetrack.yaml'):
        ...
        try:
            self.key_frame_limit = max(
                1, int(app_config.get('processor.key_frame_limit') or 3)
            )
        ...
        if crop is not None:
            if blur_variance is not None:
                ...
                self._update_key_frames(
                    self.tracks[track_id], crop, frame_time, bbox, frame_score
                )
                ...
                self.tracks[track_id]['best_frame'] = crop
                self._update_key_frames(
                    self.tracks[track_id], crop, frame_time, bbox, 0.0
                )
