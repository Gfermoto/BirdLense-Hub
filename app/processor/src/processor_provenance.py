...
def build_pipeline_fingerprint(app_config) -> dict[str, Any]:
    ...
    config_digest = hashlib.sha256(
        json.dumps(config_slice, sort_keys=True, default=str).encode('utf-8')
    ).hexdigest()
    ...
    return {
        ...
        'binary_model': _model_record(
            app_config.get('processor.models.binary')
        ),
        'classifier_model': _model_record(
            app_config.get('processor.models.classifier')
        ),
        'fusion': {
            'enabled': bool(
                app_config.get('detection.use_learned_fusion') or False
            ),
