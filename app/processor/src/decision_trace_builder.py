...
def build_decision_trace_payload(
    *,
    ...
    review_only_count = sum(
        ...
        if str(item.get('outcome_bucket') or '') == 'review_only'
    )
    ...
    trace: dict[str, Any] = {
        ...
        'recording_context': {
            ...
            'frigate_activity_hold_seconds': (
                recording_context or {}
            ).get('frigate_activity_hold_seconds'),
    ...
    accepted_trace_rows = [
        decision_trace_row(item, persisted_to_clip=True)
        for item in persisted_tracks
    ]
    rejected_trace_rows = [
        decision_trace_row(item, persisted_to_clip=False)
