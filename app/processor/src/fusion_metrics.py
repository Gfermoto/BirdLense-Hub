...
def evaluate_binary_scores(
    ...
    *,
    ...
    for threshold in thresholds:
        covered = [(s, y) for s, y in zip(scores, labels) if s >= float(threshold)]
        coverage = len(covered) / total if total else 0.0
        precision = mean([y for _s, y in covered]) if covered else 0.0
        recall = sum(y for _s, y in covered) / sum(labels) if covered and sum(labels) > 0 else 0.0
    ...
    accuracy = mean([int((s >= 0.5) == bool(y)) for s, y in zip(scores, labels)]) if scores else 0.0
    return {
        "n": total,
