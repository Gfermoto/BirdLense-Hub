...
| **`PROCESSOR_SECRET`** | Заголовок `X-Processor-Token` для processor → web | При рассинхроне: **403** в логах процессора; см. статус. Одно значение в **`app/.env`** (один контейнер). |
...
| **`BIRDLENSE_RECORDINGS_MIRROR_SFTP_PASSWORD`** | Опциональное **рантайм**-перекрытие пароля зеркала из UI/YAML (#350) | Если не задан — берётся значение из настроек / `user_config.yaml`. |
| **`BIRDLENSE_RECORDINGS_MIRROR_SFTP_KEY_PASSPHRASE`** | Опциональное перекрытие пасфразы ключа | Тот же принцип. |
...
4. **Откат**
   - Восстановить **`app/.env`** (и YAML) из бэкапа.
...
- Web и processor в одном контейнере читают **один** **`app/.env`** — одна ротация, один перезапуск.
