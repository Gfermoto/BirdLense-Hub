...
| Issue | Status | Notes |
|-------|--------|--------|
| [#367](https://github.com/Gfermoto/BirdLense-Hub/issues/367) Epic | **Open (execution tracking)** | Repo-scope baseline is delivered, but child execution issues were reopened after acceptance-criteria audit: [#379](https://github.com/Gfermoto/BirdLense-Hub/issues/379), [#389](https://github.com/Gfermoto/BirdLense-Hub/issues/389), [#390](https://github.com/Gfermoto/BirdLense-Hub/issues/390), [#392](https://github.com/Gfermoto/BirdLense-Hub/issues/392). |
...
| [#389](https://github.com/Gfermoto/BirdLense-Hub/issues/389) DINOv2 production pipeline | **Open (execution pending)** | RFC/contracts/gates are shipped, but issue remains open until execution-level acceptance criteria from the issue body are fully implemented and verified. |
| [#390](https://github.com/Gfermoto/BirdLense-Hub/issues/390) Re-ID productization | **Open (execution pending)** | Policy/gates exist, but issue remains open until product-grade review/merge UX and measurable quality validation are fully implemented. |
...
- [#389] DINOv2 production path: RFC [ML_DINOV2_PRODUCTION_PIPELINE.md](ML_DINOV2_PRODUCTION_PIPELINE.md) + **offline contract enforcement** (JSONL fields → SQLite columns → import migrations → `reid_summary@v2.contract`) + phased rollout gates; execution DoD is still open.
- [#390] Re-ID product safety: RFC [ML_REID_PRODUCTIZATION.md](ML_REID_PRODUCTIZATION.md) + **config-driven gates** (`processor.reid_*`) + **`video_reid_match@v2`** UI hints + shadow/A-B validation plan; execution DoD is still open.
