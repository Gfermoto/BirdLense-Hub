...
Скрипт проверяет только **YOLO** и цепочку процессор → API. Не проверяет:
- Frigate MQTT (триггер записи по событию);
- BirdNET MQTT (аудио-детекции);
- Go2RTC (подключение к потоку).
...
См. также: [TESTING.md](./TESTING.md), [DEPLOYMENT.md](./DEPLOYMENT.md), [MQTT_DISCOVERED_TOPICS.md](./MQTT_DISCOVERED_TOPICS.md).
