...
`app/.env.example` may keep **example** private IPs. Maintainer-only local overrides (untracked deploy env files, personal SSH aliases) stay out of git—only committed templates apply to everyone.
