...
- **Jetson Nano (aarch64)** — NVIDIA SoC: SD root + SSD data, NVIDIA runtime, TensorRT / NVDEC-NVENC later; no OpenVINO IR requirement.
...
|--------|---------------------|-------------|
| **Arch** | x86_64 | aarch64 |
| **Dockerfile** | `app/Dockerfile` (`ultralytics/ultralytics:8.4.21`) | `app/Dockerfile.jetson` (`nvcr.io/nvidia/l4t-base:r32.7.1` verified; DeepStream gated by NGC/native install) |
| **Compose** | `docker-compose.yml` + optional `docker-compose.override.yml` (Intel DRI) | `docker-compose.yml` + `docker-compose.jetson.yml` |
| **Inference** | OpenVINO IR + `intel:gpu` (optional) | TensorRT/DeepStream planned; current base validates L4T GPU runtime |
| **Weights** | `.pt` + `*_openvino_model/` | `.pt` / ONNX / TRT `.engine` on SSD |
...
| **Env flags** | `BIRDLENSE_INFERENCE_BACKEND=openvino`, `BIRDLENSE_OPENVINO_BINARY_ENABLED=1` | `BIRDLENSE_PLATFORM=jetson_nano`, `BIRDLENSE_OPENVINO_BINARY_ENABLED=0` |
...
- **Positive:** Jetson deploy target stays clean: runtime allowlist instead of full dev repository.
- **Negative:** Jetson video acceleration still CPU-only until `ffmpeg_nvdec` / NVENC / DeepStream integration lands.
- **Follow-up:** TensorRT export path, `capture_backend: ffmpeg_nvdec`, NGC/native DeepStream gate, optional `requirements-jetson.txt` without OpenVINO pip on arm64.
