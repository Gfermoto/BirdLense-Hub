...
| `single_stage_coco_bird_only_auto` | Default **true**: if single_stage loads a model with **exactly 80** classes (typical COCO, e.g. `yolov8n.pt`), detect only the **bird** class — avoids spurious objects (knife, toilet, …). Set **false** for a custom 80-class detector that is not COCO. |
