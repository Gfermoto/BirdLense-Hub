...
| `BIRDLENSE_RECORDINGS_MIRROR_SFTP_PASSWORD` | SFTP password for `storage.recordings_mirror` (NAS copy of recordings; issue #350) |
| `BIRDLENSE_RECORDINGS_MIRROR_SFTP_KEY_PASSPHRASE` | Passphrase for the SSH private key when `storage.recordings_mirror.ssh_private_key_path` points to an encrypted key |

**NAS / SFTP mirror (recordings):** With `storage.recordings_mirror.enabled: true`, the processor uploads each saved session folder to SFTP in the background after finalize. Paths in the database stay `data/recordings/...`; the hub still serves video from local disk unless you enable **`delete_local_after_success`** (expert-only: breaks playback until files exist locally again). **Alternative:** mount NAS at `DATA_DIR` / bind-mount `recordings/` — no SFTP code path. See [INSTALL.md](./INSTALL.md) data paths.
