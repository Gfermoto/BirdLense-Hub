...
| `require_auth_for_video_stream` | **`false`** (default): guests can play recordings (`/api/ui/videos/:id/stream`), consistent with [ACCESS_CONTROL](./ACCESS_CONTROL.md). **`true`**: stream requires Contributor/Admin (legacy lock-down). |
...
**Sources and capabilities:** `mqtt` and `esphome_mqtt` are **MQTT-backed** modes: the **processor** subscribes to weight topics, writes `feeder_scale_state.json` / `feeder_scale_history.jsonl`, can **estimate per-clip delta**, and can optionally **start recording** on a weight spike. `esphome_direct` polls the device over the ESPHome Web API and is intended for **live weight / bird_present / tare only**. `homeassistant` uses HA REST for the **current weight in the feeder card only**. The processor does **not** poll direct ESPHome or HA for history, so history / delta / motion-trigger options apply only to **MQTT-backed** sources.

| Key | Description |
|-----|-------------|
| `integrations.scales.enabled` | Feeder / smart-scale weight path (default **false**). |
| `integrations.scales.source` | `mqtt` (default) — manual MQTT topics; `esphome_mqtt` — same MQTT pipeline but UX preset for BirdLense ESPHome firmware; `esphome_direct` — ESPHome Web API (live weight / bird_present / tare only); `homeassistant` — HA REST snapshot in web only. |
...
| `integrations.scales.esphome_weight_sensor_id` | ESPHome `sensor` id for weight in `esphome_direct` mode. Hub reads `GET /sensor/<id>`. |
| `integrations.scales.esphome_bird_present_sensor_id` | Optional ESPHome `binary_sensor` id for bird presence in `esphome_direct` mode. Hub reads `GET /binary_sensor/<id>`. |
| `integrations.scales.esphome_tare_button_id` | Optional ESPHome `button` id for tare in `esphome_direct` mode. Hub calls `POST /button/<id>/press`. |
...
| `integrations.scales.weight_estimate_enabled` | When **true** (default), the processor may store a **weight delta for the recording window** on the video row. **Independent** of `motion_trigger_enabled`: you can estimate weight on clips started by Frigate/motion without auto-start from scales. Requires an **MQTT-backed** source (`mqtt` / `esphome_mqtt`) and `feeder_scale_history.jsonl` under `DATA_DIR`. The delta is **not** saved when the clip only has BirdNET rows (`source=audio`) with no frame/track: audio helps species ID; it is not tied to feeder weight. |
...
**Stack like [ESPHome + Home Assistant smart scale](https://github.com/igiannakas/Homeassistant-scale-with-auto-tare-and-object-detection?tab=readme-ov-file#hardware-setup)** (HX711, ESP32, proximity, auto-tare in HA): tare and “object on platform” stay in **ESPHome/HA**. BirdLense can be wired three ways:
- `mqtt`: subscribe the processor to the same MQTT state topic your integration publishes (often `homeassistant/sensor/<sensor_id>/state`)
- `esphome_direct`: poll the device over ESPHome Web API for live weight / bird_present / tare
- `homeassistant`: current weight only in the web UI via HA REST

**BirdLense ESPHome MQTT firmware (repo `bird-feeder-scale.yaml`):** use `source: esphome_mqtt`. Usually `mqtt_topic_prefix: frigate` is enough; the hub derives **`frigate/weight`**, **`frigate/bird_present`**, and **`frigate/command`** automatically. If you need a mixed layout, keep `source: esphome_mqtt` but override `mqtt_topic`, `mqtt_bird_present_topic`, or `mqtt_command_topic` explicitly.
