...
[MCP](https://modelcontextprotocol.io/) lets compatible editors and other clients call BirdLense Hub tools backed by your OpenAPI surface.
...
Add the server entries below in your editor or client’s MCP settings (exact file path depends on the product). **Never commit** tokens or secrets.
...
For clients that should read **Markdown in the GitHub repo** (`docs/`, `README`, etc.) without a running Hub, add [GitMCP](https://gitmcp.io):
