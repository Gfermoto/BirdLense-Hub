...
|-------|-------------------|--------|
| Motion / capture source | Legacy `motion.source`; trigger blocks under **Settings → Capture & Feeder** | Processor reads merged config; UI may show nested paths after save — compare audit vs YAML export. |
...
- Deeper RFC (single trigger schema, deprecating `motion.source`): track separately; this page is **inventory + pointers**.
