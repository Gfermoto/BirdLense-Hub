...
**Rule of thumb for new routes:** return a **stable machine key** (`code` or `error_code`) **plus** a human string; let the client map to i18n when needed. Document the shape in [OpenAPI](https://github.com/Gfermoto/BirdLense-Hub/blob/main/app/web/openapi.yaml) / Redoc.
...
|-------|-------------------|
| **Bandit** + **pip-audit** | `make ci-local` / `scripts/ci-full-local.sh`; pip-audit ignore `PYSEC-2022-42969` is documented in [CI_AND_QUALITY](./ci-and-quality.md). |
| **Secrets** | Never commit real `.env`; use `app/.env.example` and deploy templates. |
| **CORS** | Production `CORS_ORIGINS` / public URL — [DEPLOY_SERVER](../user/deploy-server.md) and operator runbooks. |
