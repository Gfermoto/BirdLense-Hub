...
| `DATABASE_URL` | Optional. SQLAlchemy URI. Default: SQLite under `DATA_DIR`. For high write load use PostgreSQL, e.g. `postgresql+psycopg://user:pass@host:5432/dbname`. Operator runbook: [POSTGRES_MIGRATION](../../archive/internal/docs-legacy/POSTGRES_MIGRATION.md). |
...
| `BIRDLENSE_HIDE_DIRECT_RECORDINGS` | `1` / `true` / `yes` / `on` — omit nginx `location` for `/data/recordings/` so anonymous **`GET /data/recordings/...`** falls through to **403**; playback remains **`/api/ui/videos/:id/stream`**. Default: if `BIRDLENSE_ENV=production` and `BIRDLENSE_STRICT_API_AUTH` is enabled, direct recordings are hidden automatically; otherwise static alias stays enabled unless this flag is set. **Public/VPS checklist:** [PUBLIC_RECORDINGS.md](./public-recordings.md). |
...
| `BIRDLENSE_INFERENCE_BACKEND` | Overrides `processor.inference_backend` (`torch`, `openvino`, …) — see [CV_ML_ROADMAP_PHASES](../../archive/internal/docs-legacy/CV_ML_ROADMAP_PHASES.md) |
...
**Heimdall dashboard:** URL checklist and manual tile setup (Heimdall v2 has no bulk import in the UI) — [HEIMDALL](../../archive/internal/docs-legacy/HEIMDALL.md).
...
| `detector_scope` | First-stage detector targets. Default: `["Bird", "Rodent"]`. EU classifier non-bird class **Rodent** is the catalog name; raw detector weights may still label that head “Squirrel”, which the hub maps to **Rodent**. Background / hard-negative detector classes must stay outside this scope; see [CV / ML prep contract](../../archive/internal/docs-legacy/CV_ML_PREP.md). |
...
**Operational rotation** (backup, restart, verification, rollback): [SECRETS_ROTATION.md](../../archive/internal/docs-legacy/SECRETS_ROTATION.md).
...
[INSTALL](./install.md) · [ARCHITECTURE](../contributor/architecture.md) · [ACCESS_CONTROL](../contributor/access-control.md) · [API](../contributor/api.md) · [SCENARIOS](./scenarios.md) · [GLOSSARY](./glossary.md) · [SECRETS_ROTATION](../../archive/internal/docs-legacy/SECRETS_ROTATION.md) · [PUBLIC_RELEASE_CHECKLIST](../../archive/internal/docs-legacy/PUBLIC_RELEASE_CHECKLIST.md)
