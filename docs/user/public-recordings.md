...
[Russian](../ru/public-recordings.ru.md)
...
Other **`/data/*`** paths (DB, datasets, cache) are **not** served as static files — see [SECURITY.md §3](../contributor/security.md).
...
| 2 | **`BIRDLENSE_STRICT_API_AUTH=1`** + [production gates](https://github.com/Gfermoto/BirdLense-Hub/blob/main/AGENTS.md#production-gates) | Locks mutating **`/api/ui/*`**; many read-only dashboard GETs remain public per [ACCESS_CONTROL](../contributor/access-control.md). |
...
Details stay in [SECURITY.md §3](../contributor/security.md) for threat framing; **this page** is the operator “what to turn on” SSOT.
...
- [DEPLOY_SERVER.md §8](./deploy-server.md) — short pointer from deploy checklist  
- [CONFIGURATION.md](./configuration.md) — **`BIRDLENSE_HIDE_DIRECT_RECORDINGS`**, **`require_auth_for_video_stream`**  
- [ACCESS_CONTROL.md](../contributor/access-control.md) — strict **`/api/ui/*`** allowlists  
