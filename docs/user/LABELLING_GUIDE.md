...
- `/recordings -> Video Details` — семантика записи: `Вид + Кличка + Поведение`.
...
- Блок вида позволяет:
  - исправить `Species`,
  - задать/изменить `Nickname` (ReID),
  - выбрать `Behavior` из расширенного словаря.
...
- Цель: сверять прогнозы и собирать чистую обратную связь оператора на уровне семантики в карточке видео.
