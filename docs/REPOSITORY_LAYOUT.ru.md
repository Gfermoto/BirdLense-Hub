...
| Путь | Назначение |
|------|------------|
| **`app/web/`** | Flask, REST API, OpenAPI (`openapi.yaml`). Обработчики: `routes/` — `ui_routes.register_routes`, доменные `ui_*_routes`, `ui_system_*`, `processor_routes` ([ARCHITECTURE.ru.md](./ARCHITECTURE.ru.md)). |
