...
**Проверка в CI:** `python3 scripts/check_site_map_meta_paths.py` (нужен PyYAML из `requirements-docs.txt`) убеждается, что каждый путь из секций **Use the hub**, **Develop & integrate**, **ML & project**, **Meta** и **Repository (canonical files)** в `mkdocs.yml` по-прежнему встречается в соответствующих отрывках `docs/SITE_MAP.md` до `mkdocs build --strict`.
