...
| 6   | i18n в UI                                                                                    | [#52](https://github.com/Gfermoto/BirdLense-Hub/issues/52) ✅ locale switch + `zh` (简体中文)                                                                      | P3, web                  |
...
| 13  | Detector: не-птицы (грызуны, кошки и т.д.)                                                                                                | [#163](https://github.com/Gfermoto/BirdLense-Hub/issues/163) ✅ issue закрыт; трекер — [консилиум п.17](#detection-strategy-consilium); новый issue при старте датасета/модели | P3, processor, research                                      |
...
| 18  | **ML / видео: распознавание действий** птицы (прилёт, отлёт, кормление, питьё, драка и т.п.) — **любая** установка: кормушка, парк/заповедник с IP-камерой и др.; отдельно от классификации вида по кадру | [#379](https://github.com/Gfermoto/BirdLense-Hub/issues/379) — **открытая execution-задача** (подбор и интеграция модели) + research-контекст; рядом: [#157](https://github.com/Gfermoto/BirdLense-Hub/issues/157) (запись), [#164](https://github.com/Gfermoto/BirdLense-Hub/issues/164) (виды). **Research:** [§ ниже](#bird-behavior-ml-research) | P3, processor, ML, research |


<h3 id="bird-behavior-ml-research">Research: действия птиц на видео (кратко)</h3>

Похожее **в науке и данных есть**; готового универсального «действия птицы из коробки» под **вашу** сцену (кормушка любой сложности, точка в парке/на валунах, заповедник по IP) в открытом виде — мало: обычно свой датасет, дообучение или гибрид сигналов (в т.ч. весы только там, где они есть).

| Направление | Комментарий |
|-------------|-------------|
| **Visual WetlandBirds** (2025, [Scientific Data](https://www.nature.com/articles/s41597-025-05516-5), [arXiv](https://arxiv.org/abs/2501.08931), [код](https://github.com/3dperceptionlab/visual-wetlandbirds)) | **178** роликов, **858** клипов поведения (~**20 с** среднее, ~**59 мин** суммарно), **13** видов, **7** поведений: Alert, Feeding, Flying, Preening, Resting, Swimming, Walking — bbox **по кадрам** + вид. Бейзлайны поведения (accuracy): Video ResNet **0,56**, MViT/Swin **0,51**, TimeSFormer **0,49**, S3D **0,29**; детекция вида (YOLOv9): mAP50 **0,801**. Домен съёмки — валуны Испании (**не** тот же визуальный мир, что уличная кормушка или фиксированная точка в парке). Подробности — [#379](https://github.com/Gfermoto/BirdLense-Hub/issues/379). |
| **Авто-этограммы / сегменты событий** | **Kagu nest** ([Dryad](https://datadryad.org/dataset/doi:10.5061/dryad.kh18932bb), IJCV [2023](https://doi.org/10.1007/s11263-023-01781-2)): **~253 ч** @25 FPS (**~23M** кадров), bbox + **5** событий (Feeding, Pushing/Throwing leaves, Walk-In/Out) + освещение; стриминговая сегментация без обрезки клипов. См. [#379](https://github.com/Gfermoto/BirdLense-Hub/issues/379). |
| **Практика под хаб** | Часто: **трек** (bbox) + клип-классификатор (TSN / SlowFast / VideoMAE) или **слабые признаки** (на кормушке — весы + детекция; в парке — только vision/правила по треку). Явные «прилёт/отлёт» везде требуют разметки или эвристик по треку и границам клипа. |
...
1. **Хаб (`user_config.yaml`):** `integrations.scales.enabled: true`. Для прошивки из репо лучше **`source: mqtt`** и `mqtt_topic_prefix: birdlense/scale`, тогда хаб сам берёт `…/weight`, `…/bird_present`, `…/command`; **`source: esphome`** — прямой ESPHome Web API для live-снимка без журнала/триггера по MQTT.
...
- Вне волны D: [#164](https://github.com/Gfermoto/BirdLense-Hub/issues/164) (классификатор); для HA/Heimdall актуальны только [#231](https://github.com/Gfermoto/BirdLense-Hub/issues/231) (observe-only MQTT discovery) и [#230](https://github.com/Gfermoto/BirdLense-Hub/issues/230) (docs). [#229](https://github.com/Gfermoto/BirdLense-Hub/issues/229) и [#233](https://github.com/Gfermoto/BirdLense-Hub/issues/233) закрыты как отклонённые направления; [#234](https://github.com/Gfermoto/BirdLense-Hub/issues/234) ✅ — [плитки Heimdall](./HEIMDALL.ru.md).

**Архитектура и сопровождаемость (следующая волна, апр. 2026):** на той же доске [BirdLense Hub — Roadmap](https://github.com/users/Gfermoto/projects/2) — [#292](https://github.com/Gfermoto/BirdLense-Hub/issues/292) (декомпозиция `app/web/app.py`), [#293](https://github.com/Gfermoto/BirdLense-Hub/issues/293) (сервисный слой / тонкие роуты), [#281](https://github.com/Gfermoto/BirdLense-Hub/issues/281) (Pydantic на mutating API), [#294](https://github.com/Gfermoto/BirdLense-Hub/issues/294) (N+1 / индексы), [#295](https://github.com/Gfermoto/BirdLense-Hub/issues/295) (**базовая линия ✅** — bootstrap + ABC `DetectionStrategy` + `DetectionStrategyProtocol`, см. [ARCHITECTURE.ru.md](./ARCHITECTURE.ru.md) § Базовая линия maintainability), [#296](https://github.com/Gfermoto/BirdLense-Hub/issues/296) (TanStack Query на основных экранах — **фаза 1**; контекст / prop drilling — в issue), [#297](https://github.com/Gfermoto/BirdLense-Hub/issues/297) (CI: сложность, OpenAPI→TS; политика npm audit → **#284** ✅). **Security / hardening:** [#277](https://github.com/Gfermoto/BirdLense-Hub/issues/277)–[#286](https://github.com/Gfermoto/BirdLense-Hub/issues/286). **#284** ✅ **закрыт** — по расписанию **npm audit** для `app/ui`; [TESTING.ru.md](./TESTING.ru.md). **#287** ✅ **закрыт** — аудит: в `create_app`/рантайме web нет `ALTER TABLE`; DDL только через Alembic; см. [ARCHITECTURE.ru.md](./ARCHITECTURE.ru.md) § База данных.
