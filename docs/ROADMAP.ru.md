...
|                 | Версия                                                                                                               |
| --------------- | -------------------------------------------------------------------------------------------------------------------- |
| **Ultralytics** | **8.4.33** в рантайме (`app/processor/requirements.txt`, ставится через `pip` в образе). Базовый образ Docker — **`ultralytics/ultralytics:8.4.21`** (более новый базовый тег ломал сборку ngx_brotli под nginx в CI; см. [CHANGELOG](../CHANGELOG.md) **0.3.1**). |
