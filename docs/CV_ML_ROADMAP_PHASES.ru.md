...
Инференс и бенчмарки ведём в **`ML`** до ревью; **`dev`** не перегружаем — с неё проще откатить рабочий сервер. Слияние **`ML` → `main`** (или в `dev`, когда решите) через PR после зелёного CI; деплой — `make deploy`, когда придёт время.
