...
Пошаговая инструкция: классификатор птиц на GPU T4 бесплатно. Датасеты и скрипты: [DATASETS](./DATASETS.ru.md). Локально собранный **классификаторный** merge обычно в **`datasets/`** в корне репо (в `.gitignore`), напр. **`datasets/merged_cls`**. **Детектор:** куда пишет merge, зачем **`brg/`**, чем HF-ZIP отличается от `BirdLense_detector_brg_*.zip` — блок **Актуальные пути** в начале [DATASETS.ru.md](./DATASETS.ru.md).
