...
new_dataset_path = 'nabirds_yolo_reduced'
...
# Function to get parent class if the name contains "("
def get_parent_class(class_id, hierarchy):
    while '(' in dataset_config['names'][class_id]:
        class_id = hierarchy[class_id]
    return class_id
...
def process_split(images_dir, labels_dir):
    for label_file in os.listdir(labels_dir):
        ...
        for label in labels:
            ...
            new_class_id = get_parent_class(class_id, hierarchy)
            valid_classes[new_class_id] += 1
...
def process_images_and_labels(split_images_dir, split_labels_dir, new_split_images_dir, new_split_labels_dir):
    for label_file in os.listdir(split_labels_dir):
        ...
        for label in labels:
            class_id = int(label.split()[0])
            new_class_id = old_to_new_class_id.get(get_parent_class(class_id, hierarchy))
