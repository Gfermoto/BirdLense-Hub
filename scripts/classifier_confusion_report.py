#!/usr/bin/env python3
"""Classifier confusion + threshold hints from operator corrections.

Usage (repo root):
  make classifier-confusion-report
  python scripts/classifier_confusion_report.py \
    --db app/data/db/birdlense.db --json
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Classifier confusion from species_correction log"
    )
    parser.add_argument(
        "--db",
        type=Path,
    ...
    calib = report.get("calibration_metrics") or {}
    topk = report.get("topk_metrics") or {}
    print("\nCalibration summary:")
    print(f"  ECE: {calib.get('ece')}")
    print(f"  Brier: {calib.get('brier')}")
    print(f"  Top-1 before: {topk.get('top1_before')}")
    print(f"  Top-3 proxy before: {topk.get('top3_proxy_before')}")
    print(
        "  Unknown share (policy): "
        f"{topk.get('unknown_share_after_policy')}"
    )
    ...
    if rec.get("bird_skip_classifier_doc"):
        print(
            ...
            f"{rec['bird_skip_classifier_doc']}"
        )
    return 0
