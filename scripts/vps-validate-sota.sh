...
make verify DEPLOY_URL="$DEPLOY_URL"
...
curl -sf "$DEPLOY_URL/api/ui/health" >/dev/null
curl -sf "$DEPLOY_URL/api/ui/reid/gallery/status" -H "Cookie: ${BIRDLENSE_SESSION_COOKIE:-}" || true
