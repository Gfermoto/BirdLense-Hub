...
detector = detectNet(detector_file, threshold=0.5, labels=labels, input_blob="input_0", output_bbox="boxes", output_cvg="scores", argv=[f'--log-level=info'])
detector.SetTrackingEnabled(True)

video_capture = jetson_utils.videoSource("./videos/video4.mp4", argv=['--headless'])
video_output = jetson_utils.videoOutput("./videos/test_detector.mp4")
