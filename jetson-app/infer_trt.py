...
alloced_images = [jetson_utils.cudaAllocMapped(
    width=224, height=224, format="rgb8") for _ in range(3)]
...
def main():
    ...
    with open(TRT_ENGINE, 'rb') as f, trt.Runtime(TRT_LOGGER) as runtime:
        ...
        h_output1 = cuda.pagelocked_empty(trt.volume(
            (batch_size, 1000)), dtype=trt.nptype(DTYPE))
    ...
    with engine.create_execution_context() as context:
        ...
        context.set_binding_shape(0, (batch_size,) + INPUT_SHAPE)
        ...
        for i in range(LOOP_TIMES):
            ...
            prep_time = time.time()
            print('Preprocessing time {}: {} [msec]'.format(
                i, (prep_time - st) * 1000))
            context.execute(
                bindings=[int(preprocessed_image.ptr), int(d_output1)])
            print('Inference time {}: {} [msec]'.format(
                i, (time.time() - prep_time) * 1000))
            print('Total inference time {}: {} [msec]'.format(
                i, (time.time() - st) * 1000))
            print('---', i)
            ...
            predictions = np.squeeze(h_output1)
