...
class FrameProcessor:
    def __init__(self):
        ...
        self.detector = detectNet(detector_file, threshold=0.5, labels=labels, input_blob="input_0", output_bbox="boxes", output_cvg="scores", argv=[f'--log-level=info'])
    ...
    def run(self, img):
        ...
        self.logger.debug(f'Detection Time: {(time.time() - st) * 1000} [msec]')
        if len(detections) == 0:
            return
        
    
        # Batch classification
        detections = [det for det in detections if det.TrackID != -1 and det.TrackStatus == 1 and det.TrackFrames >= MIN_TRACK_FRAMES]
        if len(detections) == 0:
            return
        ...
        for det in detections:
            crop_roi = (int(det.Left), int(det.Top), int(det.Right), int(det.Bottom))
            bird_img = cudaAllocMapped(width=crop_roi[2] - crop_roi[0], height=crop_roi[3] - crop_roi[1], format=img.format)
            # crop the image to the ROI
            cudaCrop(img, bird_img, crop_roi)
            imgs.append(bird_img)
        self.logger.debug(f'Cropping Time : {(time.time() - st) * 1000} [msec] (detections: {len(detections)})')
            
        st = time.time()
        class_descs = self.classifier.classify(imgs)
        self.logger.debug(f'Classification Time : {(time.time() - st) * 1000} [msec] (images: {len(imgs)})')

        for i in range(len(imgs)):
            det = detections[i]
            class_desc = class_descs[i]
            self.logger.debug(f'Track Info: Track ID: {det.TrackID}, Status: {det.TrackStatus}, Frames: {det.TrackFrames}, Lost: {det.TrackLost}')
    ...
    def get_results(self):
        fps = 10
        tracks_to_preds = {track_id: preds for track_id, preds in self.tracks_to_preds.items() if len(preds) >= 5 * fps}
        most_common_preds = {track_id: Counter(preds).most_common(1)[0][0] for track_id, preds in tracks_to_preds.items()}
